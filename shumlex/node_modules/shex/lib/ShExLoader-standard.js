    ...
    return f === '-'
      ? new Promise(function (resolve, reject) {
        ...
        process.stdin.on('end', function () {
          resolve({text: inputChunks.join(''), url: f})
        })
    ...
    : new Promise(function (resolve, reject) {
      ...
      FS.readFile(filename, 'utf8', function (error, text) {
        if (error) {
          ...
          resolve({text: text, url: fileURLmatch ? f : 'file://' + Path.resolve(process.cwd(), f)})
        }
      })
    })
  ...
  function LoadExtensions () {
    return FS
      ...
      .reduce(function (ret, moduleDir) {
        var path = Path.join(__dirname, '../extensions/', moduleDir, '/module')
        try {
          var t = require(path)
          ret[t.url] = t
          return ret
